		...
		<section>
			...
			<div class="bg-white px-8 py-8 pt-4 shadow-md">
				...
				<div v-show="normalUser">
					<user-buttons :id="criminalId" :criminals="criminals" :respondentName="this.respondentName">
...
import { user, app, assetPath } from './scripts/api.js';
...
computed : { 
	...
	defaultAvatar(){
		return assetPath + '/assets/images/default_avatar.jpg';
	},
	...
	avatarPath(){
		if (this.criminals.photo){
			...
			let string = assetPath + `/assets/images/default_avatar.jpg`;
	...
	criminalBounty(){
		var bounty ; 
		if ( this.criminals.profile == null ){
			bounty = "No Bounty for this criminal yet";
		}
		else {
			bounty = this.criminals.profile.bounty +" " +this.criminals.profile.currency;
		}
		return bounty ;
